...
#include "PSLP_status.h"
    ...
    */
    ...
    /* Map a primal-dual point (x, y) of the original problem to the reduced
       problem, e.g. to warm start a solver on the reduced problem. Must be
       called after 'run_presolver' has returned UNCHANGED or REDUCED.
       ...
       the mapped point and z_red (length reduced_prob->n) holds
       z_red = c_red - A_red^T y_red. Any output pointer may be NULL to skip
       that part; x_red requires x, y_red requires y, and z_red requires y_red.
       The function does not modify presolver->sol.

       If (x, y) is an optimal primal-dual pair of the original problem and
       'relax_bounds' is false, (x_red, y_red, z_red) is an optimal primal-dual
       point of the reduced problem, up to presolve tolerances. For an arbitrary
       point the result is a starting point, not necessarily feasible or
       optimal. After 'free_presolver_reduced_problem', x_red is not projected
       onto the reduced bounds and z_red is skipped (with a warning). */
